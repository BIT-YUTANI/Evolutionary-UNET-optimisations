...
input_dataset = Path('/kaggle/input/datasets/shivanshsingh059/gen-5-6-3')
...
CONFIG = {
    ...
    'dna_space': {
        ...
        'growth_ratio':     {'type': 'cont', 'min': 1.0, 'max': 2.0},           # ← Max 1.8 (was 2.5)
    ...
    'evolution': {
        'population_size':  15,        # Population per generation
        'generations_per_block': 5,   # ← Each block runs 5 generations
        'crossover_prob':   0.7,
        'mutation_prob':    0.3,
        'tournament_size':  5,
        'proxy_epochs':     20,        # ← Reduced for speed
        'proxy_samples':    300,      # ← Reduced to prevent OOM
        'proxy_val_samples': 75,
...
print("   2. Run Block 2 repeatedly to continue evolution (5 gens per run)")
...
"""
================================================================================
BLOCK 2: EVOLUTIONARY SEARCH - GENERATIONS 6-10 (RESUME)
✅ SESSION-INDEPENDENT - Works in new notebook after checkpoint restore
================================================================================
"""
...
print("=" * 80)
print("  BLOCK 2: Evolutionary Search (Continuation – runs next 5 gens)")
print("=" * 80)

# ============================================================================
# ✅ INITIALIZE DEAP (needed for new session)
# ============================================================================

print("\n🔧 Initializing DEAP for resumed session...")

# Remove existing if present (for clean restart)
...
print("✅ DEAP toolbox initialized")

# ============================================================================
# ✅ EVALUATION FUNCTION (same as Block 1)
# ============================================================================
...
def evaluate_proxy(individual):
    ...
    try:
        ...
        if not validate_param_budget(estimated_full):
            print(f"   ⚠️  Model too large: {estimated_full/1e6:.1f}M params")
            del model
            cleanup_memory()
...
# ============================================================================
# ✅ EVOLUTION RUNNER (same as Block 1)
# ============================================================================
...
# %%
# ============================================================================
# ✅ AUTO-DETECT LATEST CHECKPOINT & COMPUTE GENERATION RANGE
# ============================================================================
# This notebook is designed to be run repeatedly. Each run it:
#   1. Scans for the highest-numbered evo_blockN.pkl
#   2. Reads the last completed generation from gen_df
#   3. Runs the next 5 generations
#   4. Saves as evo_block(N+1).pkl with the full accumulated history

import re as _re

def find_latest_evo_checkpoint():
    """Return (block_num, filename) for the highest evo_blockN.pkl found."""
    ckpt_dir = CONFIG['checkpoint_dir']
    pattern = _re.compile(r'^evo_block(\d+)\.pkl$')
    candidates = []
    for f in ckpt_dir.glob('evo_block*.pkl'):
        m = pattern.match(f.name)
        if m:
            candidates.append((int(m.group(1)), f.name))
    if not candidates:
        return None, None
...
print("\n" + "=" * 80)
print("  📂 Auto-detecting Latest Evolution Checkpoint")
print("=" * 80)

latest_block_num, latest_ckpt_name = find_latest_evo_checkpoint()

if latest_block_num is None:
    raise FileNotFoundError(
        "❌ No evolution checkpoint found!\n"
        "   Make sure you:\n"
        "   1. Restored checkpoints from your saved dataset (first cell)\n"
        "   2. Ran Block 0 (CONFIG) first\n"
        "   3. At least evo_block1.pkl must exist in:\n"
        f"      {CONFIG['checkpoint_dir']}"
    )

print(f"\n✅ Found latest checkpoint: {latest_ckpt_name}  (block #{latest_block_num})")

# ── Load previous data ────────────────────────────────────────────────────────
prev_data = load_checkpoint(latest_ckpt_name)
if not prev_data:
    raise RuntimeError(f"❌ Failed to load checkpoint: {latest_ckpt_name}")

prev_gen_df  = prev_data['gen_df']
prev_eval_df = prev_data['eval_df']

# Compute the next generation window dynamically
last_completed_gen = int(prev_gen_df['generation'].max())
gens_per_block     = CONFIG['evolution']['generations_per_block']
start_gen          = last_completed_gen + 1
end_gen            = last_completed_gen + gens_per_block
next_block_num     = latest_block_num + 1
next_ckpt_name     = f'evo_block{next_block_num}.pkl'
next_stats_prefix  = f'evo_block{next_block_num}'

print(f"   Last completed generation : {last_completed_gen}")
print(f"   Next block will run       : Gen {start_gen}–{end_gen}")
print(f"   Will save as              : {next_ckpt_name}")

# ============================================================================
# ✅ FIX 1: Restore population & HOF WITHOUT losing fitness
#
# ROOT CAUSE OF THE 54-MINUTE "RESTORING FITNESS" WASTE:
#   creator.Individual(existing_ind) creates a BRAND NEW object from the
#   gene values (iterating them into a new list). The new object has NO
#   .fitness attribute set, so fitness.valid = False for every individual.
#   This forced full re-evaluation of all 15 population members = 54 min wasted.
#
# THE FIX:
#   DEAP's Individual is a list subclass. pickle() preserves both the gene
#   values AND the .fitness.values tuple on the existing objects.
#   Simply use list() to get the unpickled objects directly — no re-creation.
# ============================================================================

print(f"\n🔄 Restoring population from Gen {last_completed_gen}...")

# ✅ CORRECT: reuse the pickled objects — fitness is fully preserved
resume_pop = list(prev_data['pop'])
resume_hof = prev_data['hof']

# Verify fitness is intact (should always be 0 invalid with the fix above)
_invalid_pop = [ind for ind in resume_pop if not ind.fitness.valid]
_invalid_hof = [ind for ind in resume_hof if not ind.fitness.valid]

print(f"✅ Checkpoint loaded")
print(f"   Population size : {len(resume_pop)}  ({len(_invalid_pop)} need re-eval)")
print(f"   Pareto front    : {len(resume_hof)} individuals  ({len(_invalid_hof)} need re-eval)")
print(f"   Previous best IoU : {prev_gen_df['max_iou'].max():.4f}")

# Only re-evaluate the truly invalid ones (should be zero after the fix)
if _invalid_pop:
    print(f"\n⚠️  Re-evaluating {len(_invalid_pop)} population individuals with missing fitness...")
    for ind in tqdm(_invalid_pop, desc="Fixing pop"):
        ind.fitness.values = evaluate_proxy(ind)
        K.clear_session()
        gc.collect()
        time.sleep(0.3)
...
if _invalid_hof:
    print(f"\n⚠️  Re-evaluating {len(_invalid_hof)} HOF individuals with missing fitness...")
    for ind in tqdm(_invalid_hof, desc="Fixing HOF"):
        ind.fitness.values = evaluate_proxy(ind)
        K.clear_session()
        gc.collect()
        time.sleep(0.3)

if not _invalid_pop and not _invalid_hof:
    print("✅ All individuals have valid fitness — no re-evaluation needed (saved ~54 min)")

# ============================================================================
# RUN NEXT BLOCK OF GENERATIONS (DYNAMIC)
# ============================================================================

print("\n" + "=" * 80)
print(f"  Starting Evolution: Generations {start_gen}–{end_gen}")
print("=" * 80)

# Check if this exact block was already completed (safe resume)
resume_data = load_checkpoint(next_ckpt_name)
if resume_data:
    print(f"\n♻️  Found {next_ckpt_name} – this block was already completed.")
    pop_final        = list(resume_data['pop'])
    hof_final        = resume_data['hof']
    eval_df_combined = resume_data['eval_df']
    gen_df_combined  = resume_data['gen_df']
else:
    # ✅ No mass re-evaluation needed — fitness already valid from pickle
    pop_final, hof_final, eval_df_new, gen_df_new = run_evolution_block(
        start_gen, end_gen,
        resume_pop=resume_pop,
        resume_hof=resume_hof,
    )

    # Accumulate full history
    eval_df_combined = pd.concat([prev_eval_df, eval_df_new], ignore_index=True)
    gen_df_combined  = pd.concat([prev_gen_df,  gen_df_new],  ignore_index=True)

    # ── Save checkpoint ───────────────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"  💾 SAVING CHECKPOINT: {next_ckpt_name}")
    print(f"{'='*70}")

    checkpoint_data = {
        'pop':     pop_final,
        'hof':     hof_final,
        'eval_df': eval_df_combined,
        'gen_df':  gen_df_combined,
    }
    save_checkpoint(checkpoint_data, next_ckpt_name)

    ckpt_path = CONFIG['checkpoint_dir'] / next_ckpt_name
    if ckpt_path.exists():
        print(f"   ✅ Checkpoint verified!")
        print(f"   📁 {ckpt_path}")
        print(f"   📊 {ckpt_path.stat().st_size / (1024*1024):.2f} MB")

    save_csv(eval_df_combined, f'{next_stats_prefix}_evaluations.csv')
    save_csv(gen_df_combined,  f'{next_stats_prefix}_gen_stats.csv')

# ============================================================================
# ✅ FIX 2: OOM-SAFE CHAMPION SELECTION
#
# ROOT CAUSE OF THE KERNEL CRASH (the actual bug from the log):
#   The old code ran evaluate_proxy() on EVERY HOF member unconditionally
#   (disguised behind a validity check that always returned True because
#   fitness was lost by the creator.Individual() reconstruction above).
#   Each evaluation builds a TF model. Without cleanup between calls,
#   GPU memory accumulates: 14s → 30s → 49s → 78s → 109s → 138s → 164s → DEAD
#
# THE FIX:
#   1. With Fix 1 above, fitness IS preserved → the check correctly returns
#      False for all HOF members → zero re-evaluations needed in normal flow.
#   2. If any truly invalid individuals DO exist (edge case), we evaluate them
#      ONE AT A TIME with aggressive GPU cleanup between each call:
#      K.clear_session() + gc.collect() + time.sleep(0.5)
#      This prevents the exponential memory accumulation that caused the crash.
# ============================================================================

total_gens = int(gen_df_combined['generation'].max())

print("\n" + "=" * 80)
print(f"  BLOCK COMPLETE: Generations 1–{total_gens} Total")
print("=" * 80)

best_gen = int(gen_df_combined.loc[gen_df_combined['max_iou'].idxmax(), 'generation'])
print(f"\n📊 Overall Results (Gen 1–{total_gens}):")
print(f"   Total evaluations: {len(eval_df_combined)}")
print(f"   Pareto front     : {len(hof_final)} individuals")
print(f"   Best IoU ever    : {gen_df_combined['max_iou'].max():.4f}  (Gen {best_gen})")
print(f"   Final best IoU   : {gen_df_combined.iloc[-1]['max_iou']:.4f}")

print("\n🏆 Selecting Champion Architecture...")

# ✅ SAFE: only evaluate individuals that GENUINELY have no fitness
_hof_needs_eval = [ind for ind in hof_final
                   if not ind.fitness.valid or len(ind.fitness.values) < 3]

if _hof_needs_eval:
    print(f"🔄 {len(_hof_needs_eval)}/{len(hof_final)} HOF individuals need re-evaluation...")
    for ind in tqdm(_hof_needs_eval, desc="Validating HOF"):
        ind.fitness.values = evaluate_proxy(ind)
        # ✅ CRITICAL: aggressive cleanup between each model build
        # Without this, GPU memory accumulates: 14s→30s→49s→78s→...→CRASH
        K.clear_session()
        gc.collect()
        time.sleep(0.5)
else:
    print(f"✅ All {len(hof_final)} HOF individuals have valid fitness — skipping re-evaluation entirely")

valid_hof = [ind for ind in hof_final
             if ind.fitness.valid and len(ind.fitness.values) >= 3]

if not valid_hof:
    print("⚠️  No valid HOF individuals – falling back to best in population")
    valid_hof = [ind for ind in pop_final
                 if ind.fitness.valid and len(ind.fitness.values) >= 3]

if valid_hof:
    champion_ind    = max(valid_hof, key=lambda x: x.fitness.values[0])
    champion_dna    = decode_individual(champion_ind)
    champion_iou, champion_f1, champion_params = champion_ind.fitness.values

    print(f"\n🏆 CHAMPION ARCHITECTURE (selected from Gen 1–{total_gens}):")
    print(f"   Proxy IoU  : {champion_iou:.4f}")
    print(f"   Proxy F1   : {champion_f1:.4f}")
    print(f"   Est Params : {abs(champion_params):.1f}M")
    print(f"   DNA        : {dna_summary(champion_dna)}")
    print(f"   Filters    : {dna_to_filters(champion_dna)}")

    champion_save_data = {
        'dna':                  champion_dna,
        'proxy_iou':            float(champion_iou),
        'proxy_f1':             float(champion_f1),
        'estimated_params_M':   float(abs(champion_params)),
        'filters':              dna_to_filters(champion_dna),
        'filters_proxy':        dna_to_filters(champion_dna, half=True),
        'selected_from_gen':    total_gens,
    }

    save_json(champion_save_data, 'champion_dna.json')

    save_checkpoint({
        'champion_dna':        champion_dna,
        'champion_fitness':    (float(champion_iou), float(champion_f1),
                                float(champion_params)),
        'champion_individual': list(champion_ind),
    }, 'champion.pkl')

    print("\n✅ Champion saved successfully!")

else:
    raise RuntimeError("❌ Could not find any valid individuals for champion selection!")

next_start = total_gens + 1
next_end   = total_gens + gens_per_block
print(f"\n💡 Options:")
print(f"   • Run THIS notebook AGAIN to continue → Gen {next_start}–{next_end}")
print(f"   • Switch to Optuna notebook if you have enough generations")
print("💾 Don't forget to save this session's checkpoints as a Kaggle dataset!")
...
5. Name it: 'change-detection-checkpoints-block6' (or appropriate block number)
6. Click 'Create'
